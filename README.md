...
| `resume-salary-variants` | 在保留两页测试工程师 PDF 简历版式与事实边界的前提下处理标注修订、通篇校对，并生成不同期望月薪的投递版本。 |
...
### 7. resume-salary-variants

用于处理已有两页中文测试工程师 PDF 简历的内容优化、标注修订与薪资投递变体。它保留原始模块顺序、侧栏风格、字号层级和事实边界，重点检查自我评价短尾行、标题间距、正文对比度、项目四个子模块以及两页内的排版完整性。

- 脚本：`resume-salary-variants/scripts/create_salary_variants.py`
- 默认薪资版本：`8k`、`8-9k`、`9k`、`9-10k`、`10k`、`10-11k`、`11k`、`12k`
- 使用方式：以已经审核通过的基准 PDF 为输入，仅覆盖第一页的“期望月薪”字段；第 2 页及其他正文保持不变。
